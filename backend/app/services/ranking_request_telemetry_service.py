...
class RankingRequestTelemetryService:
    async def log(
        ...
        *,
        ...
        requested_ranking_mode: Optional[str] = None,
        ...
        shadow_model_version_id: Optional[str] = None,
        ...
        event = RankingRequestTelemetry(
            ...
            requested_ranking_mode=(requested_ranking_mode or None),
            ranking_mode=(ranking_mode or None),
            experiment_key=(experiment_key or None),
            experiment_variant=(experiment_variant or None),
            rollout_variant=(rollout_variant or None),
            rollout_bucket=None if rollout_bucket is None else max(0, min(int(rollout_bucket), 9999)),
            rollout_percent=None if rollout_percent is None else max(0, min(int(rollout_percent), 100)),
            shadow_mode=(shadow_mode or None),
            shadow_model_version_id=(shadow_model_version_id or None),
            shadow_candidate_count=max(0, int(shadow_candidate_count)),

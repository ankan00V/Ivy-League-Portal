...
class DriftService:
    async def run(
        ...
        *,
        ...
        await report.insert()
        if metrics_module.MODEL_INPUT_DRIFT_VALUE is not None:
            metrics_module.MODEL_INPUT_DRIFT_VALUE.labels(metric="query_bucket_psi").set(float(metrics["query_bucket_psi"]))
            metrics_module.MODEL_INPUT_DRIFT_VALUE.labels(metric="max_feature_mean_z").set(float(metrics["max_feature_mean_z"]))
            metrics_module.MODEL_INPUT_DRIFT_VALUE.labels(metric="alert").set(1.0 if alert else 0.0)
            metrics_module.MODEL_INPUT_DRIFT_VALUE.labels(metric="impressions").set(float(metrics["impressions"]))
        return report

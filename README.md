...
- The employer portal is retired, kept on disk and unmounted. Its only gate was a non-freemail email address, which let anyone with a bought domain post into the candidate feed.
...
- Expand the staging E2E matrix (success + failure + recovery paths).

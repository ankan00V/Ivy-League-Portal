...
**The academician feed is still served by keyword fallback, not yet by academician sources.** 14 faculty sources qualify - AICTE, four NITTTRs, ICMR, DST, IIT Bombay/Delhi/Kanpur/Madras, IISc, JNU, ICSSR, INFLIBNET. One of them, IISc, has produced candidates: 12 rows, held in probation. The rest yield nothing, and not because the pipeline is broken - IIT Bombay's careers page qualifies at 88.5 and its extraction learns a parser at 0.851 confidence, then returns zero rows. Indian academic recruitment is largely published as PDF notices inside government-CMS table layouts, and the extractor is built for structured job boards.

The IISc rows also showed why probation exists. Of twelve, three were real openings - a postdoctoral programme, an instructor post, a faculty recruitment drive - and the rest were the paperwork of past recruitments: admit cards, merit lists, results, provisional eligibility lists. Those sit in the same markup as the vacancies and often the same wording minus one noun, so they are now filtered by title at extraction rather than left to be promoted. Nothing reached the feed; probation is where it was caught.

Until faculty sources produce reliably, the endpoint reports the split - `from_faculty_sources` against `from_keyword_fallback` - so the claim can be checked rather than assumed.
